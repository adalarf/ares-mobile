...
export const MiniGamesScreen = ({ navigation }) => {
    const handleRandomTraining = async () => {
        try {
            ...
            let response = await fetch('http://10.0.2.2:8000/training/random_exercise', {
                method: 'GET',
                headers: {
            ...
            if (data === null) {
                ...
                response = await fetch('http://10.0.2.2:8000/training/random_exercise', {
                    method: 'POST',
                    headers: {
                        'Authorization': `Bearer ${token}`,
                        'Content-Type': 'application/json',
    ...
    return (
        <View style={{ flex: 1 }}>
            <ImageBackground source={require('../assets/training-background.png')} style={styles.backgroundImage} />
            <View style={styles.header}>
                <TouchableOpacity onPress={() => navigation.navigate('mainPage')}>
                    <Image source={require('../assets/back-icon.png')} style={styles.menuIcon} />
                </TouchableOpacity>
            ...
            <View style={styles.cardsWrapper}>
                <ScrollView contentContainerStyle={styles.cardsContainer} showsVerticalScrollIndicator={false}>
                    <TouchableOpacity style={styles.card}>
                        <ImageBackground source={require('../assets/blitz-poll-icon.png')} style={styles.cardImage} imageStyle={styles.cardImageStyle}>
                    ...
                    <TouchableOpacity style={styles.card} onPress={handleRandomTraining}>
                        <ImageBackground source={require('../assets/random-training-icon.png')} style={styles.cardImage} imageStyle={styles.cardImageStyle}>
                            <View style={styles.cardContent}>
                                <Text style={styles.cardTitle}>Случайная тренировка</Text>
                                <Text style={styles.cardSubtitle}>Справишься с любой тренировкой?</Text>
                            </View>
                        </ImageBackground>
                    </TouchableOpacity>
                </ScrollView>
